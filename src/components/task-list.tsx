// Step 7: IntersectionObserver で無限スクロールを実装
// Step 8: ステータス変更ボタンの onClick を接続
    ...
    >
      ...
      <div className="flex items-center gap-2 mt-3">
        <button
          onClick={(e) => {
            e.stopPropagation();
            // Step 8 で実装: updateTask.mutate({ id: task.id, updates: { status: nextStatus[task.status] } })
          }}
...
export function TaskList() {
  ...
  const loadMoreRef = useCallback(
    (node: HTMLDivElement | null) => {
      ...
      if (!node) return;
      // ← Step 7: ここに IntersectionObserver を作成して node を observe する
    },
